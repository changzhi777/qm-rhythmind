"""
tests/unit/test_metrics_agent.py — MetricsAgent 单元测试
...
class TestMetricsAgentHappyPath:
    ...
    async def test_run_returns_metrics_analysis(
        self, mock_persist, mock_qmd, mock_influx, agent_ctx, user_id, session_id
    ):
        """正常流程：写入成功 + 返回 MetricsAnalysis 结构"""
        mock_qmd.return_value = []

        agent = MetricsAgent(user_id=user_id, influx=mock_influx)
    ...
    async def test_write_metrics_called_with_correct_point(
        self, mock_persist, mock_qmd, mock_influx, agent_ctx, user_id
    ):
...
class TestAnomalyDetection:
    ...
    async def test_no_anomaly_for_normal_data(
        self, mock_persist, mock_qmd, mock_influx, user_id, session_id
    ):
        mock_qmd.return_value = []
        normal_data = {
...
class TestStaticMethods:
    ...
    def test_parse_metrics_filters_none(self):
        ...
        m = MetricsAgent._parse_metrics(raw)
    ...
    def test_detect_anomalies_empty_metrics(self):
        result = MetricsAgent._detect_anomalies({k: None for k in _ANOMALY_RULES})
        assert result == []

    def test_classify_load_boundaries(self):
        assert MetricsAgent._classify_load(0) == "very_low"
        assert MetricsAgent._classify_load(4.9) == "very_low"
        assert MetricsAgent._classify_load(5.0) == "low"
        assert MetricsAgent._classify_load(19.9) == "low"
        assert MetricsAgent._classify_load(20.0) == "moderate"
        assert MetricsAgent._classify_load(49.9) == "moderate"
        assert MetricsAgent._classify_load(50.0) == "high"
        assert MetricsAgent._classify_load(79.9) == "high"
        assert MetricsAgent._classify_load(80.0) == "very_high"
        assert MetricsAgent._classify_load(200.0) == "very_high"
